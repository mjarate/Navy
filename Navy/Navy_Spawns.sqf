...
Navy_ChooseRandomTriggerPosition =
{
	...
	//DECLARE(_position) = getPos _trigger; // [x,y,z?]
	//DECLARE(_random_position) = [[_position ,_trigger],"ground"] call BIS_fnc_randomPos;
	DECLARE(_random_position) = [triggerArea _trigger,getposATL _trigger,true] call adm_common_fnc_getRandomPositionInArea;
	DEBUG
	{
		[["Random position: %1 chosen from trigger: %2 at position: %3",_random_position,_trigger,_position]] call Navy_Debug_HintRPT;
	};
	_random_position;
};