...
navy_method_fnc_heliinsert = {
    ...
    DEBUG {
        [["Spawning helicopter: %1 on side: %2 with pilot: %3 in trigger: %4 for routine: Heli Insert", _vehicleClassname, _side, _pilotClassname], DEBUG_INFO] call navy_debug_fnc_log;
    };
    ...
    waitUntil {
        sleep 1;
        ALTITUDE(_vehicle) < 15;
    };
    [_vehicle, NAVY_OPEN_DOOR] call navy_main_fnc_animateDoors;
    waitUntil {
        sleep 0.5;
        ((ALTITUDE(_vehicle) < 0.5) || (isTouchingGround _vehicle));
    };
    {
        unassignVehicle _x;
