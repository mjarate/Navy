...
navy_method_fnc_barrelbomb = {
    ...
    DECLARE(_paradropDistance) = [NAVY_CONFIG_FILE, "Settings", "bombingDistance"] call navy_config_fnc_getNumber;
    ...
    DECLARE(_paradropWP) = [_pilot, _waypoints, "Barrel_bomb", 1] call navy_main_fnc_addWaypoint;
    DECLARE(_deleteWP) = [_pilot, _waypoints, "Barrel_bomb", 2] call navy_main_fnc_addWaypoint;
    waitUntil {
        sleep 2;
        DEBUG {
            [["Vehicle: %1 Distance from barrelbomb point: %2 Limit: %3", _vehicle, (_vehicle distance (getWPPos _paradropWP)), _paradropDistance], DEBUG_INFO] call navy_debug_fnc_log;
        };
...
for "_i" from 1 to _cargoAmount step 1 do {
    ...
    private _barrel = createVehicle ["Land_WaterBarrel_F", [getPosATL _vehicle select 0, getPosATL _vehicle select 1,(getPosATL _vehicle select 2)-3],[], 0, "FLY"];  

		_barrel setVectorDirAndUp[[getPosATL _vehicle select 0, getPosATL _vehicle select 1,(getPosATL _vehicle select 2)-3],[0,90,0]];
